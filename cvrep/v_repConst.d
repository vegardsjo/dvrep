...
//RML_AlwaysType4	sim_intparam_rml_type /* robot motion library type to use (or being used) (2 or 4)*/
}
...
enum SIMX_HEADER_SIZE = 18;
enum simx_headeroffset_crc = 0;				/* 1 simxUShort. Generated by the client or server. The CRC for the message */
enum simx_headeroffset_version = 2;			/* 1 byte. Generated by the client or server. The version of the remote API software */
enum simx_headeroffset_message_id = 3;		/* 1 simxInt. Generated by the client (and used in a reply by the server) */
enum simx_headeroffset_client_time = 7;		/* 1 simxInt. Client time stamp generated by the client (and sent back by the server) */
enum simx_headeroffset_server_time = 11;	/* 1 simxInt. Generated by the server when a reply is generated. The server timestamp */
enum simx_headeroffset_scene_id = 15;		/* 1 simxUShort. Generated by the server. A unique ID identifying the scene currently displayed */
enum simx_headeroffset_server_state = 17;	/* 1 byte. Generated by the server. Bit coded: 0 set --> simulation not stopped, 1 set --> simulation paused, 2 set --> real-time switch on, 3-5: edit mode type (0=no edit mode, 1=triangle, 2=vertex, 3=edge, 4=path, 5=UI) */ 

/* Remote API command header: */
enum SIMX_SUBHEADER_SIZE = 26;
enum simx_cmdheaderoffset_mem_size = 0;			/* 1 simxInt. Generated by the client or server. The buffer size of the command. */
enum simx_cmdheaderoffset_full_mem_size = 4;	/* 1 simxInt. Generated by the client or server. The full buffer size of the command (applies to split chunks). */
enum simx_cmdheaderoffset_pdata_offset0 = 8;	/* 1 simxUShort. Generated by the client or server. The amount of data that is part of the command identification. */
enum simx_cmdheaderoffset_pdata_offset1 = 10;	/* 1 simxInt. Generated by the client or server. The amount of shift of the pure data buffer (applies to split chunks). */
enum simx_cmdheaderoffset_cmd = 14;				/* 1 simxInt. Generated by the client (and used in a reply by the server). The command, combined with the operation mode of the command. */
enum simx_cmdheaderoffset_delay_or_split = 18;	/* 1 simxUShort. Generated by the client or server. The amount of delay in ms of a streaming command, or the max. pure data size to send at once (applies to split commands). */
enum simx_cmdheaderoffset_sim_time = 20;		/* 1 simxInt. Generated by the server. The simulation time (in ms) when the command was executed (or 0 if simulation is not running) */
enum simx_cmdheaderoffset_status = 24;			/* 1 byte. Generated by the server. (1: bit 0 is set --> error in function execution on server side). The client writes bit 1 if command cannot be overwritten*/
enum simx_cmdheaderoffset_reserved = 25;		/* 1 byte. Not yet used */
